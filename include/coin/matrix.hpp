...
class Matrix {
    ...
    Matrix() = default;
    Matrix(size_type r, size_type c) : data_(r * c), rows_{r}, cols_{c} {}
    Matrix(size_type r, size_type c, const storage& d) : data_{d}, rows_{r}, cols_{c} {}
    ...
    iterator        begin(size_type row)        { return data_.begin() + row * cols_; }
    const_iterator  begin(size_type row)  const { return data_.begin() + row * cols_; }
    const_iterator  cbegin(size_type row) const { return data_.cbegin() + row * cols_; }

    iterator        end(size_type row)          { return begin(row) + cols_; }
    const_iterator  end(size_type row)    const { return begin(row) + cols_; }
    const_iterator  cend(size_type row)   const { return begin(row) + cols_; }


    reference       operator[] (size_type index) { return data_[index]; };
    const_reference operator[] (size_type index) const {
        return const_cast<reference>(static_cast<const Matrix&>(*this)[index]);
    };
    
    reference       operator() (size_type row, size_type col) { return data_[row * cols() + col]; };
    const_reference operator() (size_type row, size_type col) const {
        return const_cast<reference>(static_cast<const Matrix&>(*this)(row,col));
    };
...
using _impl_matrix::Matrix; // simple implmentation
